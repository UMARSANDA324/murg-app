...
// Mark queue item as viewed when invoice is opened
if ($orderId) {
    $update_queue = mysqli_query($con, "UPDATE sales_queue SET status='viewed', viewed_at=NOW() WHERE orderID='$orderId' AND facilityID='$facilityID'");
}
...
$first_store_query = mysqli_query($con, "SELECT s.store_name FROM orders o LEFT JOIN stocks st ON o.stockID = st.id LEFT JOIN stores s ON st.store_id = s.id WHERE o.orderID='$orderId' AND o.facilityID='$facilityID' ORDER BY o.id ASC LIMIT 1");
$first_store_row = mysqli_fetch_assoc($first_store_query);
$header_store_name = !empty($first_store_row['store_name']) ? $first_store_row['store_name'] : 'Unassigned';
?>
...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Hide Copy 2 on screen, show only during print */
        .copy-2 {
            display: none;
        }
        ...
        @media print {
            ...
            /* Show Copy 2 during print */
            .copy-2 {
                display: block;
                page-break-before: always;
            }

            /* Add page break after Copy 1 */
            .receipt-container:not(.copy-2) {
...
<body>
    ...
    <div class="receipt-container">
        ...
        <div class="meta-section">
            ...
            <div class="meta-row">
                ...
                <span class="meta-value"><?php echo htmlspecialchars($order_row['staff']); ?></span>
            </div>
            <div class="meta-row">
            ...
            <div class="meta-row">
                ...
                <span class="meta-value"><?php echo htmlspecialchars($header_store_name); ?></span>
            </div>
        </div>
    ...
    <!-- Copy 2 - Non-Price Copy -->
    <div class="receipt-container copy-2">
        <div class="header">
            <img src="../assets/img/murglogo.jpg" alt="Logo">
            <h1>MURG TEXTILE ENTERPRISES</h1>
            <div class="tagline">Dealers on Fabrics, Shadda, Swiss, Coco, Shampo, Geznar, Menlace &amp; More</div>
        ...
        <table class="items-table">
            ...
            <tbody>
                <?php
                $sql2 = mysqli_query($con, "SELECT * FROM orders WHERE orderID='$orderId' AND facilityID='$facilityID'");
                $copy2TotalQty = 0;
                while ($row2 = mysqli_fetch_array($sql2)) {
                    $qty2 = floatval($row2['quantity']);
                    $copy2TotalQty += $qty2;
                    ?>
                    <tr>
                        <td>
                            <span class="item-description"><?php echo htmlspecialchars($row2['item']); ?></span>
                        </td>
                        <td class="text-right"><?php echo number_format($qty2); ?></td>
                    </tr>
                <?php } ?>
            </tbody>
        </table>
        ...
        <div class="totals-container">
            <div class="total-row main-total">
                <span>TOTAL ITEMS</span>
                <span><?php echo number_format($copy2TotalQty); ?></span>
            </div>
        </div>
        ...
        <div class="footer">
            <div class="thank-you">Goods Received Confirmation</div>
            <small>Powered by MURG TEXTILE ENTERPRISES</small>
            <small>&copy; Teemassan Tech</small>
        </div>
    </div>
